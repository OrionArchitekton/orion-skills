...
**5. Exit-2-preserving traps.** Advisory hook -> `trap 'exit 0' EXIT`. **Blocking** hook -> `trap 'rc=$?; [ "$rc" -eq 0 ] && exit 0; exit 2' EXIT` (clean -> `exit 0`; ANY non-zero, a crash included, -> `exit 2`, per rule 4). A blanket `exit 0` trap, or the `-eq 2`-only variant, leaves a fail-open hole: a crash (`rc=1`) falls through to `exit 0` = silent hole.
...
| "The hook errored, so it blocked" | Exit 1 / crash = fail-OPEN in Claude Code. Only `exit 2` or `exit 0` + `deny` blocks. |
