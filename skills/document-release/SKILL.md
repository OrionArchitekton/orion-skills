...
- If the URL contains "github.com" then platform is **GitHub**
- If the URL contains "gitlab" then platform is **GitLab**
...
(adapt to whatever project you're in; these are generic, not tied to any specific toolchain):
...
3. Compute the corrected title: strip any existing `v<X.Y.Z.W>`-style prefix (and its separator) from the front of the current title, then prepend the correct `v$V ` prefix. This one pass covers all three cases: a title that's already correct comes out identical (strip + re-prepend), a stale/different version prefix gets replaced, and a title with no prefix gets one added.

```bash
NEW_TITLE=$(echo "$CURRENT_TITLE" | sed -E 's/^v[0-9]+(\.[0-9]+){1,3}[[:space:]:-]*//')
NEW_TITLE="v$V $NEW_TITLE"
```
...
This is a documentation-accuracy discipline, not a doc generator or style guide. What counts
as "your project's docs" (README, ARCHITECTURE, CONTRIBUTING, CLAUDE.md or equivalent), your
branching and PR/MR conventions, and the shape of your CHANGELOG are your repo's own
doctrine. The invariant is the shape: never regenerate CHANGELOG history, never bump VERSION
silently, and never leave a shipped change with stale or missing docs.

## References (progressive disclosure)

To keep this skill lean, depth was moved to `references/` (load only when needed):

- Shared runtime preamble (10 sections) → `references/preamble.md`
